...
const StudentsService = {
  async getStudents(p0: { page: number; page_size: number; search: string }): Promise<Student[]> {
  ...
  async updateStudent(id: string, studentData: Partial<Student>): Promise<Student> {
    const response = await api.put<Student>(`/api/v1/student/update/${id}/`, studentData);
    return response.data;
  },
